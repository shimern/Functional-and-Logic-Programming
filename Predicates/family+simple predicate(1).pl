...
%ѕроверка €вл€етс€ ли X братом Y, причем X!=Y
brother(X,Y):- parent(P,X), parent(P,Y), man(X), not(X==Y).

%¬ывод всех братьев.ѕровер€ем по одному родителю, чтоб не было повторов.
brothers(X):- parent(P,X),parent(P,Y), man(Y),man(P), write(Y), nl, fail.

%ѕроверка €вл€етс€ ли X сестрой Y
sister(X,Y):- parent(P,X), parent(P,Y), woman(X), not(X==Y).

b_s(X,Y):-brother(X,Y). % X,Y брат-сестра или брать€
